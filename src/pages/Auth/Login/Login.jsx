...
function Login() {
  ...
  const handleLogin = (e) => {
    ...
    loginApi(user)
      .then(data => {
        if (data.user._id) {
          Cookies.set('accessToken',data.accessToken,{ expires: 30, path: '/', secure: true, sameSite: 'None' })
          navigate('/boards')
        } else {
    ...
    <Box flex='1' display='flex' justifyContent='center' alignItems='center'>
      <Box sx={{
        ...
        <Box display='flex' width='100%'>
          <Button sx={{ border:'1px solid #ccc', marginRight:'10px', width:'50%' }}><Link to='/auth/forgot' style={{ textDecoration:'none', color:'#0C66E4', width:'100%' }}>Can&apos;t login</Link></Button>
          <Button sx={{ border:'1px solid #ccc', width:'50%' }}><Link to='/auth/register' style={{ textDecoration:'none', color:'#0C66E4', width:'100%' }}>Create an account</Link></Button>
        </Box>
      </Box>
    </Box>
  )
